...
function  theFinalFunction() {
    ...
    var minHeight = $("#box_6a").outerHeight(true);
    $("#box_6").css("min-height",minHeight);
...
$(".startQuiz").on("click.timelinePause", function() {
  mm.contexts.forEach((event,i) => {
      event.data.forEach((timeline,i) => {
          //check if value is a timeline (not undefined) and is repeating
          if (typeof timeline._repeat !== 'undefined' && timeline._repeat == -1) {
              timeline.pause();
          }
      });
  });
